<script lang="ts">
	...
	const variants = $derived({
		...
		solid: `${toneSolid[tone]} ${soft}`,
		outline: `bg-bg border ${toneBorderSoft[tone]} ${edge}`
	});
...
<section class="w-full px-6 py-16 {className}" {...rest}>
	<div
		class="mx-auto flex w-full max-w-6xl gap-8 rounded-[32px] p-10 sm:p-14 {variants[variant]}
			{inline
