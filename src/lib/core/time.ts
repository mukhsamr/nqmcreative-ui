...
export function clampTime(minutes: number, min?: string, max?: string, step = 1): number {
 ...
 */
export function stepTime(value: string, direction: 1 | -1, step = 1, min?: string, max?: string) {
 ...
 */
export function formatClock(value: string, locale: string, hour12?: boolean): string {
	const minutes = parseTime(value);
	if (minutes === null) return '';

	const date = new Date(2000, 0, 1, Math.floor(minutes / 60), minutes % 60);
	return new Intl.DateTimeFormat(locale, { hour: '2-digit', minute: '2-digit', hour12 }).format(
		date
	);
}
