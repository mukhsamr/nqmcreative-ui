...
export function addTag(
	...
	if (!value) return { tags, rejected: 'empty' };
	if (value.length < minLength) return { tags, rejected: 'short' };

	const fold = (text: string) => (caseInsensitive ? text.toLowerCase() : text);
	if (tags.some((tag) => fold(tag) === fold(value))) return { tags, rejected: 'duplicate' };
	if (max && tags.length >= max) return { tags, rejected: 'full' };
 ...
 */
export function splitTags(text: string, separators: readonly string[] = [',']): string[] {
