<script lang="ts">
	...
	import { TONES, toast } from '$lib/core/index.js';
...
<div class="min-h-screen bg-bg px-6 py-16 text-text">
	<div class="mx-auto flex max-w-4xl flex-col gap-16">
		...
		<section class="flex flex-col gap-5">
			...
			<p class="font-mono text-xs text-text-muted">
				fired from a module-level store; one &lt;Toaster /&gt; is mounted at the bottom of this page
			</p>
		</section>
		...
		<section class="flex flex-col gap-5">
			...
			<div class="grid gap-6 sm:grid-cols-2">
				...
				<ui.Field label="City" hint="Type to filter, arrows to move." for="city">
					<ui.Combobox id="city" bind:value={city} options={cities} placeholder="Search cities…" />
				</ui.Field>
		...
		<section class="flex flex-col gap-5">
			...
			<div class="flex flex-wrap items-start gap-6">
				...
				<p class="max-w-xs font-sans text-sm text-text-secondary">
					<code class="font-mono text-xs">Calendar</code> stands alone, and
					<code class="font-mono text-xs">DatePicker</code> wraps it in a text field. Month and
					weekday names come from the browser; the field's part order is set by its
					<code class="font-mono text-xs">format</code> prop.
				</p>
			</div>
		</section>
