...
<section class="flex flex-col gap-4">
	...
	<CodeBlock code={code.appCss} label="src/app.css" />
	<Alert tone="warning" title="The @source line is not optional">
		Without it every component renders unstyled, with no error anywhere. Tailwind v4 skips
		node_modules, so it never sees the class names that live inside the package. Adjust the path to
...
<section class="flex flex-col gap-4">
	...
	<p class="font-sans leading-relaxed text-text-secondary">
		Each component has its own subpath — the file name in kebab-case, so
		<code class="font-mono text-sm text-brand">AvatarGroup</code> is
		<code class="font-mono text-sm text-brand">@nqmcreative/ui/avatar-group</code>.
	</p>
	<CodeBlock code={code.usage} label="+page.svelte" />
	<p class="font-sans leading-relaxed text-text-secondary">The barrel works too:</p>
	<CodeBlock code={code.barrel} label="either way" />
	<p class="font-sans text-sm leading-relaxed text-text-secondary">
...
<section class="flex flex-col gap-4">
	<h2 class="font-heading text-xl font-medium tracking-tight">Other install routes</h2>
	<h3 class="font-sans text-sm font-medium text-text">A packed tarball</h3>
	<p class="font-sans text-sm leading-relaxed text-text-secondary">
	...
	<p class="font-sans text-sm leading-relaxed text-text-secondary">
		For working on the library and an app at once. Use npm here —
		<code class="font-mono text-brand">bun link</code> and
		<code class="font-mono text-brand">bun add file:&lt;dir&gt;</code> both fail on Windows with
		<code class="font-mono text-brand">EBUSY</code>, because bun copies the whole source directory
		into its cache.
	</p>
	<CodeBlock code={code.installSymlink} label="terminal" />
</section>
