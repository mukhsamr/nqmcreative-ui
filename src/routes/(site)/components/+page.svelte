...
<div class="flex flex-col gap-10">
	<header class="flex flex-col gap-4">
		...
		<p class="max-w-xl font-sans text-text-secondary">
			{count} components, each importable on its own subpath.
		</p>
		<Input bind:value={query} placeholder="Filter components…" class="max-w-sm" />
		{#if query}
			...
			<div class="grid gap-px border border-hairline bg-hairline sm:grid-cols-2">
				{#each group.items as item (item.slug)}
					...
					>
						...
						<span class="pt-1 font-mono text-[11px] text-text-muted">{item.subpath}</span>
					</a>
				{/each}
			</div>
		</section>
